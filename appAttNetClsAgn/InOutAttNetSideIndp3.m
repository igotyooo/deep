classdef InOutAttNetSideIndp3 < handle
    properties
        db;                     % A general db.
    ...
    methods( Access = public )
        function this = InOutAttNetSideIndp3...
            ...
            this.settingTsDb.posTrunMargin                      = 0.8;      % Should be smaller than 1. [ pix/pix ]
            this.settingTsDb.posStopMargin                      = 1.2;      % Should be larger than 1. [ pix/pix ]
        ...
        function name = getName( this )
            name = sprintf( 'IOANETSIDINDP3_%s_OF_%s', ...
    ...
    methods( Access = private )
        ...
        function subTsDb = makeSubTsDb( this, setid )
            ...
            posTrunMargin = this.settingTsDb.posTrunMargin;
            posStopMargin = this.settingTsDb.posStopMargin;
            posGotoMargin = this.settingTsDb.posGotoMargin;
            numQtz = this.settingTsDb.numQuantizeBetweenStopAndGoto;
            step = sqrt( ( posGotoMargin / posStopMargin ) ^ ( 2 / ( numQtz + 1 ) ) );
            ...
            for newiid = 1 : numIm;
                ...
                for oid = 1 : numObj,
                    tlbr = oid2tlbr( :, oid );
                    tlbrTrun = scaleBoxes( tlbr, posTrunMargin, posTrunMargin );
                    tlbrStop = scaleBoxes( tlbr, posStopMargin, posStopMargin );
                    tlbrGo = zeros( 4, numQtz + 1, 'single' );
                    for g = 1 : numQtz + 1,
                        s = posStopMargin * ( step ^ g );
                        tlbrGo( :, g ) = scaleBoxes( tlbr, s, s );
                    end;
                    tlbrs = [ tlbrTrun, tlbrStop, tlbrGo ];
                    dids = [ 3, 2, ones( 1, numQtz + 1 ) ];
                    n = numel( dids );
