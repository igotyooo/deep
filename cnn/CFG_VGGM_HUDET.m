function cfg = CFG_VGGM_HUDET
    % ________________________
    % 1ST CONVOLUTIONAL LAYER.
    lid = 0;
    lid = lid + 1;
    cfg{ lid, 1 }.type = 'conv';
    cfg{ lid, 1 }.filterSize = 7;
    cfg{ lid, 1 }.filterDepth = 3;
    cfg{ lid, 1 }.numFilter = 96;
    cfg{ lid, 1 }.stride = [ 2, 2 ];
    cfg{ lid, 1 }.pad = [ 0, 0, 0, 0 ];
    ...
    cfg{ lid, 1 }.filterDepth = 96;
    cfg{ lid, 1 }.numFilter = 256;
    cfg{ lid, 1 }.stride = [ 2, 2 ];
    cfg{ lid, 1 }.pad = [ 1, 1, 1, 1 ];
    cfg{ lid, 1 }.initWScal = 1;
    cfg{ lid, 1 }.initB = 0.1;
    cfg{ lid, 1 }.filtersLearningRate = 1;
    cfg{ lid, 1 }.biasesLearningRate = 2;
    ...
    cfg{ lid, 1 }.numFilter = 512;
    cfg{ lid, 1 }.stride = [ 1, 1 ];
    cfg{ lid, 1 }.pad = [ 1, 1, 1, 1 ];
    cfg{ lid, 1 }.initWScal = 1;
    cfg{ lid, 1 }.initB = 0.1;
    cfg{ lid, 1 }.filtersLearningRate = 1;
    ...
    cfg{ lid, 1 }.type = 'conv';
    cfg{ lid, 1 }.filterSize = 1;
    cfg{ lid, 1 }.filterDepth = 2048;
    cfg{ lid, 1 }.numFilter = ( 3 + 1 + 1 ) * 2 * 1; % ( numDirection + stopSignal + noObjectSignal ) * ( TL + BR ) * numObjClass.
    cfg{ lid, 1 }.stride = [ 1, 1 ];
    cfg{ lid, 1 }.pad = [ 0, 0, 0, 0 ];
    cfg{ lid, 1 }.initWScal = 1;
    cfg{ lid, 1 }.initB = 0;
